...
parser = ArgumentParser(prog="fragment_GS", description=usage, epilog="Succes!")
...
def main():
  ...
  indir     = "/pnfs/psi.ch/cms/trivcat/store/user/ineuteli/samples"
      ...
      for mass in args.masses:
        for kappa in kappas:
          for lambd in args.lambdas:
            title    = "%s: M = %s, L = %s"%(sample,mass,lambd)
            if kappa:
              title += ", kappa = %s"%(kappa)
...
def convertGENSIM(infiles,outfilename,Nmax=-1,isPythia=False):
  ...
  tree_decay.addBranch('mother_mass', 'f')
  ...
  for event in events:
      ...
      for moth in gps_mother:       
          ...
          for i in range(moth.numberOfDaughters()):
              ...
              pair.append(dau.p4())            
              tree_decay.mother_mass[0] = moth.mass()
...
def printParticle(p):
  print "%5d, %3d: %8.2f, %5.2f, %5.2f, final %3d, mother %s"%(p.pdgId(),p.status(),p.pt(),p.eta(),p.phi(),isFinal(p),p.mother(0).pdgId())
